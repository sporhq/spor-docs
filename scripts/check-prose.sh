...
# or a tab). On what's left, drop HTML comments (which may span multiple
# lines) and the leading `!` of markdown image syntax, and strip inline code
# spans — tracked with an in_code flag that carries across lines, because a
# span opened by a single backtick commonly closes on the next line (a
# wrapped JSON example like `` `{foo,\nbar}` `` recurs throughout the API
# reference).
strip_non_prose() {
  awk '
    BEGIN { in_fence = 0; fence_char = ""; fence_len = 0; in_fm = 0; in_code = 0; code_span_len = 0; in_comment = 0 }
    {
      line = $0
      if (FNR == 1 && line == "---") { in_fm = 1; in_code = 0; code_span_len = 0; print ""; next }
      ...
      # in_code/code_span_len reset at every one of these — otherwise one
      # malformed line could blank out everything after it for the rest of
      # the file.
      if (match(line, /^ {0,3}(```+|~~~+)/)) {
        ...
        if (in_fence) {
          if (this_char == fence_char && this_len >= fence_len) {
            in_fence = 0; in_code = 0; code_span_len = 0; print ""; next
          }
        } else {
          in_fence = 1; fence_char = this_char; fence_len = this_len; in_code = 0; code_span_len = 0
          print ""; next
        }
      }

      if (in_fence) { print ""; next }
      if (match(line, /^(    |\t)/)) { in_code = 0; code_span_len = 0; print ""; next }

      # Walk the line as an alternation of backtick-runs and plain text
      # (rather than splitting on every single backtick) so a code span
      # opened by N backticks only closes on a run of exactly N, per
      # CommonMark — a double-backtick span (``like this``) is not closed by
      # a lone backtick inside it. code_span_len (the length of the run that
      # opened the span) carries across lines the same way in_code already does.
      out = ""
      pos = 1
      linelen = length(line)
      while (pos <= linelen) {
        ...
        if (match(rest, /`+/)) {
          ...
          if (!in_code) out = out pre
          if (!in_code) {
            in_code = 1
            code_span_len = run_len
          } else if (run_len == code_span_len) {
            in_code = 0
            code_span_len = 0
          }
          pos += RSTART + RLENGTH - 1
        } else {
          if (!in_code) out = out rest
          pos = linelen + 1
        }
      }

      # HTML comments can span multiple lines, so suppress content from
      # `<!--` through its matching `-->` (which may be several lines
      # later) with an in_comment flag that carries across lines like
      # in_code does, while still scanning prose before the opener and
      # after the closer on their respective lines.
      comment_out = ""
      pos = 1
      outlen = length(out)
      while (pos <= outlen) {
        ...
        if (in_comment) {
          ...
          if (match(rest, /<!--/)) {
            comment_out = comment_out substr(rest, 1, RSTART - 1)
            pos += RSTART + RLENGTH - 1
            in_comment = 1
          } else {
            comment_out = comment_out rest
            pos = outlen + 1
          }
        }
      }
