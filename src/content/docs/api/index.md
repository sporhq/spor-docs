...
- **Auth**: send `Authorization: Bearer <token>` on every request. Transport
  is HTTPS only. See [Authentication](/api/authentication/) for token kinds.
...
still read (see [Errors and compatibility](/api/errors-and-compatibility/)).
...
### [Lenses and sharing](/api/lenses-and-sharing/)
...
### [Tokens and agents](/api/tokens-and-agents/)
