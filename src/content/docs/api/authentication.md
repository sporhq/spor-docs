...
- **Self-serve**: `POST /v1/me/tokens` mints a token bound to your own
  person node. See [Tokens and agents](/api/tokens-and-agents/).
...
([Reads](/api/reads/#get-v1me)). A response with `bound: false` means the
...
([Tokens and agents](/api/tokens-and-agents/)). Authorization is ownership
...
is the bound. See [Lenses and sharing](/api/lenses-and-sharing/).
