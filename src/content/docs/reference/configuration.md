...
defaults to `file://<userConfigHome>/candidates` — this machine's own
personal config home (`$SPOR_HOME`, ignoring any repo `graph:` binding) —
even while a `graph:` marker points the shared graph itself
