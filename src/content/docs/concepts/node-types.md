...
writing its own schema nodes (see [Schemas are nodes](/concepts/schemas/)).
...
| `task` | `task-` | Active or planned work. Statuses: `open`, `active`, `done`, `abandoned`. Reaching `done` requires a resolver — see [the node model](/concepts/nodes/). |
...
| `finding` | `find-` | A gardener observation about another node, filed as a queue item — see [the gardener](/concepts/repos-and-projects/). |
...
| Type | Prefix | Role |
| --- | --- | --- |
| `lens` | `lens-` | A saved view over the graph: declarative query and render blocks, optional actions — see [Lenses](/concepts/lenses-and-workflows/). |
