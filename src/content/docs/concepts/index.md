...
| --- | --- |
| [The node model](/concepts/nodes/) | One fact per node, ids, summaries, timestamps, and the resolver gate |
| [Node types](/concepts/node-types/) | Every node type in the registry, with prefixes and roles |
| [Edge types](/concepts/edges/) | Every edge type, its direction, weight, and inverse forms |
| [The decision queue](/concepts/queue/) | Derived ranking signals, human priority, dormancy, and per-person views |
| [Briefings and corrections](/concepts/briefings/) | The compiled context packet and how to fix it when it is wrong |
| [Capture, ingestion, and questions](/concepts/capture/) | Raw text in, typed nodes out; routing questions to the person who knows |
| [Schemas are nodes](/concepts/schemas/) | Extending the ontology by writing schema nodes into the graph |
| [Local and remote mode](/concepts/local-and-remote/) | The two deployment shapes and the configuration cascade |
| [Identity and attribution](/concepts/identity/) | People, organizations, and agents acting on behalf of people |
| [Claims and leases](/concepts/claims/) | How concurrent workers avoid doing the same task twice |
| [Dispatch, capabilities, and profiles](/concepts/dispatch/) | Briefed background agents and substitution-free routing |
| [Lenses, program view, and workflows](/concepts/lenses-and-workflows/) | Saved views, the progress tree, and workflow runs |
| [Repos, projects, and the gardener](/concepts/repos-and-projects/) | Two-layer grouping and the automated hygiene sweep |
