...
For what each mode badge means, see the [overview](/cli/). For where these
verbs read their settings, see [Configuration](/cli/configuration/).
...
[Configuration](/cli/configuration/#the-credential-store)) and never clobber
a sibling tenant.
...
the local, self-serve counterpart to the admin-gated [`invite`](/cli/team-admin/#invite).
...
CI and headless use — the self-serve twin of [`invite`](/cli/team-admin/#invite),
