...
`repo:` fixes the project slug ([`spor link`](/cli/repo-scoping/#link)
...
| `SPOR_QUEUE_PROJECT` | default scope for [`spor next`](/cli/reading-the-graph/#next) (the `queue.project` config key) |
...
[`spor auth login`](/cli/getting-started/#auth) (device grant) and
[`spor join`](/cli/getting-started/#join) (paste) both add a tenant and
