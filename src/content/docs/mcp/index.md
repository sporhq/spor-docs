...
- **Search the graph** for what the team already knows about a task, before
  designing or deciding anything ([`query_graph`](/mcp/tools/#query_graph)).
- **Walk a node's neighborhood** — why it exists, what it depends on, what
  answered it ([`get_node`](/mcp/tools/#get_node), root-mode
  [`query_graph`](/mcp/tools/#query_graph), [`render_lens`](/mcp/tools/#render_lens)).
- **Read the ranked decision queue** — open work ordered by graph signal, each
  item with a one-line why ([`show_queue`](/mcp/tools/#show_queue)).
- **Write outcomes back** — a decision, a finding, a deferral — as typed,
  linked nodes the next session inherits ([`capture`](/mcp/tools/#capture),
  [`put_node`](/mcp/tools/#put_node)).
...
- **A person identity and a personal access token** (`spor_pat_…`), minted
  ...
  paste this token once — see [Connecting](/mcp/connecting/).
- **An MCP host that supports remote connectors** over Streamable HTTP with
  ...
  qualify. The [interactive widget](/mcp/widget/) additionally needs a host
...
| [`analytics`](/mcp/tools/#analytics) | Created-vs-completed, throughput, cycle time, WIP, bottlenecks |
| [`schema`](/mcp/tools/#schema) | Introspect the live schema registry — the contract as data |
| [`capture`](/mcp/tools/#capture) | Raw prose in, typed and linked nodes out — the default write door |
| [`put_node`](/mcp/tools/#put_node) | Create or update one node from full markdown |
| [`add_edge`](/mcp/tools/#add_edge) | Add one typed edge between two nodes |
| [`remove_edge`](/mcp/tools/#remove_edge) | Withdraw one edge |
| [`set_status`](/mcp/tools/#set_status) | Change one node's status, gated by the schema |
| [`set_priority`](/mcp/tools/#set_priority) | Set or clear the human priority override (p1–p3) |
| [`propose_correction`](/mcp/tools/#propose_correction) | Pin, exclude, or add guidance to future briefings |
| [`ask_question`](/mcp/tools/#ask_question) | File a question, routed to whoever stewards the closest node |
...
- [Connecting](/mcp/connecting/) — adding the connector in claude.ai or
  Claude Code, the OAuth flow you'll see, and revoking access safely.
- [The operating loop](/mcp/operating-loop/) — the ORIENT → TRAVERSE → COMMIT
  mental model the server teaches connected assistants.
- [Tool reference](/mcp/tools/) — every tool: purpose, key parameters, when
  to reach for it.
- [The widget](/mcp/widget/) — the interactive queue, lens, and program views
